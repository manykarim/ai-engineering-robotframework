...
- [x] 1.2 Scan for neutrality. Verify: a case-insensitive search of `specs/shop/` for `BUG_`, `LOCATOR_`, `stage`, `planted`, `drift`, `data-`, `css`, `xpath` and `flag` finds nothing (design D5).
- [x] 1.3 Check every concrete value used in a scenario against the image built from the tagged commit (`sha-6342d4b`, identical in source to `0.3.0`), run locally. Verify at API level:
