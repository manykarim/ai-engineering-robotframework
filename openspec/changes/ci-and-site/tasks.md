...
- [ ] 1.1 Write `tools/triage.py` (design D3). It takes `--output` (an `output.xml`), `--diff` (a file) and `--out` (the comment), and `--summary-only`. It calls the `TRIAGE_*` endpoint when all three settings are set and `--summary-only` is not given. Verify:
...
- [ ] 1.2 Add the `heal-patch` profile to `robot.toml` (design D4). Verify: `uv run robotcode -p heal -p heal-patch config show` sets `HEAL_FIX_TIER=patch`, and `-p heal` alone still sets `report`.
...
- [ ] 2.4 Write `.github/workflows/docs-site.yml` (design D6). Verify: `actionlint` passes, and every job carries the repository-name guard.
- [ ] 2.5 Check pinning and fork safety across all workflows (design D5). Verify:
...
- [ ] 3.2 Write `CONTRIBUTING.md` (design D7). Verify: it names the OpenSpec flow and the three checks, and every command in it runs.
...
- [ ] 4.1 Create `website/` with Docusaurus 3.10.2 at exact versions, a committed `package-lock.json`, and the configuration of design D6. Verify:
...
- [ ] 4.2 Check the site's safeguards. Verify:
  ...
  - no file outside `website/` was changed to make the site build.
...
- [ ] 5.1 Align Lab 9 with design D3 and D8: the secrets that add an analysis, the tiers, and no maintainer comment. Verify: `tools/check_labs.py` passes, and every workflow and secret name the lab uses exists.
...
- [ ] 5.4 Add the triage secrets to `SETUP.md` (the Claude credential and `TRIAGE_*`, optional, with the spending-cap warning). Verify: `setup-check`'s guide references still resolve.
