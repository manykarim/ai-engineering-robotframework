...
How you work:
1. Run through RobotCode, so that `robot.toml` applies: `uv run robotcode robot` for the whole suite, or with a path, `--include <tag>` or `--test "<name>"`. Add `-p shared` when the person works on the shared instance.
...
Report what ran, how many tests passed and failed, and for each failure: the test, its message, and the keyword that failed.
